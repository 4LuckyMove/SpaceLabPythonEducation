...
"""
Абстракция (abstraction) -      выделение классов объектов, при котором объекты разного
...
Инкапсуляция (encapsulation) -  сокрытие данных внутри объекта.
Наследование (inheritance) -    иерархия классов, посредством которого экземпляры
                                класса получают доступ к данным и методам классов-предков без их повторного
                                определения;
Полиморфизм (polymorphism) -    множественное наследование, позволяющее использовать один и тот же
                                интерфейс для различных действий; полиморфной переменной, например,
                                может соответствовать несколько различных методов.

Что такого "абстрактного" в каждом принципе ООП?
    Абстракция утверждает следующее: "Объект может быть рассмотрен с общей точки зрения".
    А Инкапсуляция от себя добавляет: "И это единственная точка зрения,
        с которой вы вообще можете рассмотреть этот объект."
    В Наследовании перенимает утверждение Абстракции и также добавляет собственные утверждения.
    В Полиморфизме перенимает утверждение Абстракции, но может данное утверждение менять под свои нужды.
"""
...
auto.print_status()
print("--> auto.set_color('черный')")
auto.set_color('черный')
